...
export default async function TeacherAttendancePage({ params }) {
  ...
  let dbUser = null;
  try {
    dbUser = await prisma.user.findUnique({ where: { authId: user.id }, include: { teacher: true } });
  } catch {}
  ...
  try {
    ...
    if (classSubject) {
      ...
      if (!isAssignedTeacher && !isTimetableTeacher) {
        classSubject = null;
      } else {
  ...
  const todaysLecture = pastLectures.find((lecture) => {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#1e233d] pb-6">
        <div>
          ...
          <p className="text-zinc-400 text-sm mt-1">
            {scheduledSlots.length > 0 ? `Time Slot: ${scheduledSlots.map((slot) => slot.timeSlot).join(', ')}` : 'Attendance marking and lecture notes'}
          </p>
        </div>
            ...
            {pastLectures.length === 0 ? (
              ...
              <div className="space-y-3 max-h-[700px] overflow-y-auto pr-1">
                {pastLectures.map((lecture) => {
                  const present = lecture.attendance.filter((item) => item.status === 'PRESENT').length;
                  const late = lecture.attendance.filter((item) => item.status === 'LATE').length;
                  const absent = lecture.attendance.filter((item) => item.status === 'ABSENT').length;
                  const total = lecture.attendance.length;
                  const isPending = lecture.topic.startsWith('Pending');
                  return (
                    <div key={lecture.id} className={`bg-[#0d0f1a] border rounded-xl p-4 space-y-3 ${isPending ? 'border-amber-500/20' : 'border-[#1e233d]'}`}>
                      <div className="flex justify-between items-start gap-2">
                        <div>
                          <div className="text-[10px] font-bold text-cyan-400 font-mono">
                            {new Date(lecture.date).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' })}
                          </div>
                          {isPending ? (
                            <div className="text-[11px] text-amber-400 mt-1 italic">Lecture notes pending...</div>
                          ) : (
                            <div className="text-xs text-white mt-1 whitespace-pre-line line-clamp-3">{lecture.topic}</div>
                          )}
                        </div>
                      </div>
                      {lecture.pictureUrl && !isPending && (
                        <Image
                          ...
                          height={224}
                          unoptimized
                          className="h-28 w-full rounded-lg border border-[#1e233d] bg-black/30 object-contain"
                        />
                      )}
